  ...
  const fetchDeposits = async () => {
    try {
      ...
      const response = await fetch(
        `/api/user-dashboard/deposit?userId=${user.id}`,
      );
          ...
          <div className="max-w-7xl mx-auto space-y-10">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="bg-card border border-border rounded-[1rem] p-5 md:p-7 shadow-sm">
                  ...
                  <div className="space-y-4 mb-10">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                      <CreditCard className="w-3 h-3" /> 01. Enter Deposit
                      Amount
                    </label>
                        ...
                        <button
                          ...
                          className={`flex items-center justify-between cursor-pointer px-5 py-3  rounded-xl border-2 transition-all duration-300 ${
                            selectedMethod === method.name
                              ? "bg-foreground border-none shadow-xl"
                              : "bg-background border-border hover:border-muted-foreground/50"
                          }`}
                        >
                          <div className="text-left">
                            <p
                              className={`text-[10px] font-black uppercase tracking-widest ${selectedMethod === method.name ? "text-background/60" : "text-muted-foreground"}`}
                            >
                              {method.ticker}
                            </p>
                            <p
                          ...
                          <div
                            className={`w-5 h-5 hidden lg:flex rounded-full border-2 items-center justify-center ${
                              selectedMethod === method.name
                                ? "border-background"
                                : "border-muted"
                            }`}
              ...
              <div className="w-full lg:w-[380px] space-y-5">
                {/* Total Funded Card */}
                <div className="bg-foreground text-background p-4 md:p-4 rounded-[1rem] relative overflow-hidden group">
